...
export const Navbar: React.FC<NavbarProps> = ({ onToggleMobileMenu }) => {
  ...
  return (
    <header className="h-16 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30 shadow-xs transition-all">
      ...
      <div className="flex items-center gap-3 flex-1 max-w-lg">
        ...
        <button
          ...
          className="relative w-full flex items-center bg-slate-100/90 hover:bg-slate-200/80 border border-slate-200/90 hover:border-slate-300 rounded-xl px-3.5 py-2 text-xs text-slate-500 transition-all shadow-2xs group"
        >
          <Search className="w-4 h-4 text-slate-400 mr-2 group-hover:text-teal-600 transition-colors" />
          <span className="truncate font-medium text-slate-600">Buscar paciente, tutor, microchip, DNI...</span>
          <span className="hidden sm:flex items-center gap-1 ml-auto text-[10px] text-slate-500 border border-slate-300 bg-white px-2 py-0.5 rounded-md font-mono font-semibold shadow-2xs">
      ...
      <div className="flex items-center gap-2.5 sm:gap-3.5">
        ...
        {criticalCount > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-red-50 hover:bg-red-100 border border-red-200 text-red-700 text-xs font-black transition-all shadow-2xs animate-pulse"
          >
        ...
        {/* Branch Selector */}
        <div className="hidden lg:flex items-center gap-2 text-slate-700 bg-slate-50/90 border border-slate-200 px-3 py-1.5 rounded-xl shadow-2xs hover:border-slate-300 transition-all">
